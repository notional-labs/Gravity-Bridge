...
var (
	...
	// EthAddrs holds ethereum addresses
...
func SetupTestChain(t *testing.T, weights []uint64, setDelegateAddresses bool) (TestInput, sdk.Context) {
	...
	for i, weight := range weights {
		...
		require.NoError(t, input.BankKeeper.MintCoins(input.Context, gravitytypes.ModuleName, weightCoins))
		require.NoError(t, input.BankKeeper.SendCoinsFromModuleToAccount(input.Context, gravitytypes.ModuleName, accAddr, weightCoins))
		...
		if setDelegateAddresses {
			...
			ethAddr, err := gravitytypes.NewEthAddress(gethcommon.BytesToAddress(bytes.Repeat([]byte{byte(i)}, 20)).String())
			if err != nil {
				panic("found invalid address in EthAddrs")
			}
